...
import { Heart, Building2, Music, PartyPopper, ArrowRight, Star } from 'lucide-react';
import SectionTitle from '../ui/SectionTitle';

// Map slug to icon for services from Base44
const iconMap = {
  'wesela': Heart,
  'eventy_firmowe': Building2,
  'koncerty_gale': Music,
  'imprezy_prywatne': PartyPopper
};
...
// Get icon for service - works with both default (has icon) and Base44 data (no icon)
const getServiceIcon = (service) => {
  if (service.icon) {
    return service.icon;
  }
  return iconMap[service.slug] || Star;
};
                  ...
                  <div className="md:w-2/3 p-6 md:p-8 flex flex-col justify-center">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-12 h-12 bg-amber-500/10 flex items-center justify-center">
                        <ServiceIcon className="w-6 h-6 text-amber-500" />
                      </div>
